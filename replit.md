...
- Section layout preferences: `coachSections`, `reviewSections`, `homeSections`, and `habitsHubSections` in store (local-only, not server-synced); each section has `{ id, visible }` config; `toggleSectionVisibility(page, id)` and `moveSection(page, id, direction)` actions (page: 'coach' | 'review' | 'home' | 'habitsHub'); CSS `order` on wrapper divs for reordering; gear icon in Coach/Review/Home/HabitsHub headers opens layout sheet; migration-safe (missing sections appended from defaults); store version 5
- Coach Daily/Weekly filter: `coachViewFilter` local state ('daily'|'weekly', default weekly); daily view hides weekly-only sections (narrative, advisory, drift, trajectory, gravity, evidence, streaks, proof, alignment); weekly view shows all sections; toggle UI below Coach header
- Task card context: task cards in Tasks.tsx and Home.tsx show linked goal name (emerald badge) and linked habit name (blue badge) when present
...
- Custom Domains: `customDomains: string[]` in store (v6); `DEFAULT_DOMAINS` is fixed 7-domain array; `getAllDomains()` returns `[...DEFAULT_DOMAINS, ...customDomains]`; all domain selects/filters across Tasks, Identity, Review, HabitsHub, Coach, narrativeEngine use `allDomains`; domain manager in HabitsHub (Tag icon) allows create/rename/delete; default domains are read-only; `removeCustomDomain` cascades (unlinks from habits/tasks/goals); `renameCustomDomain` cascades; persisted via api.setSetting("customDomains") + hydrated from `/api/state`
- Identity page: Active Identity cards include values, commitments, and domain emphasis inline (formerly separate "Identity Commitments" section removed)
...
- Friction Events: `frictionEvents[]` in store; captured on habit skip/task defer (max 1/day); `frictionPromptShownToday` tracks daily limit; `frictionPromptFrequency` in NotificationSettings ('normal'/'light'/'off'); 9 reason tags: Time, Energy, Environment, Emotion, Overload, Forgetfulness, Unclear, Too Large, Resistance; engine generates 7 suggestion types: reschedule, add-cue, minimum-version, reduce-frequency, create-supporting-task, break-down, explore-resistance
